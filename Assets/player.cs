...
public class player : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            aimTarget.Translate(new Vector3(0, 0, -h) * speed * 2 * Time.deltaTime); //translate the aiming gameObject on the court horizontallly
        }
    ...
    {
        if (collision.gameObject.CompareTag("Ball")){ // if we collide with the ball 
            ...
            Vector3 V = dir.normalized * currentShot.hitForce + new Vector3(0, currentShot.upForce, 0);
            collision.gameObject.GetComponent<Rigidbody>().velocity = V;
            ...
            if (ballDir.z <= 0)                                   // on out right or left side 
            {
